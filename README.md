# MoCat to Resolume: a working document

We build [MoCat](https://bigskyinteractive.com), a camera-based body
...
The MoCat half is Verified. Most of the Resolume half is Documented and
...
found no learn mode for an incoming address. So `/mocat/2d/right_wrist/y`
...
| Address | Value |
|---|---|
| `/mocat/person` | `1.0` while a person is tracked |
| `/mocat/2d/<joint>/x` `/y` | **already 0 to 1** across the picture, mirrored. `+x` right, **`+y` down** |
| `/mocat/2d/<joint>/vis` | tracker confidence, 0 to 1 |
| `/mocat/hand/left/<0-20>/x` `/y` | 21 hand points, 0 to 1, mirrored |
| `/mocat/hand/right/<0-20>/x` `/y` | the same for the other hand |
| `/mocat/3d/valid` | `1.0` when the metric solve is good |
| `/mocat/3d/position/x` `/y` `/z` | hip midpoint, **metres** |
| `/mocat/3d/rotation/x` `/y` `/z` | body orientation, **degrees** |
| `/mocat/3d/distance` | **metres** from the camera |
| `/mocat/3d/<joint>/x` `/y` `/z` | per-joint position, **metres** |
| `/mocat/3d/<joint>/vis` | confidence, 0 to 1 |

Two things that catch people:

**The 2D family is already normalised.** It is not pixels. If you have read
anywhere that MoCat sends screen pixels, that was us being wrong in a draft.
...
going quiet, not for a flag: `vis` stops too. Gate on `/mocat/person`.
...
MIT, see [LICENSE](LICENSE). MoCat is made by
...
[mocat-receiver-kit](https://github.com/BigSkyInteractive/mocat-receiver-kit),
[mocat-fluid-body](https://github.com/BigSkyInteractive/mocat-fluid-body),
[mocat-puppet-2d](https://github.com/BigSkyInteractive/mocat-puppet-2d).
